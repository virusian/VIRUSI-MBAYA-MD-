...
adams({ nomCom: "gpt", reaction: "🦠", categorie: "abu" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    if (!arg || arg.length === 0) {
      return repondre('Hello am Virusi🖐️.\n\n What help can I offer you today?');
    }
...
adams({ nomCom: "nature", reaction: "🦗", categorie: "VIRUSI 🦠 PICS" }, async (dest, zk, commandeOptions) => {
    const { repondre, arg, ms } = commandeOptions;
  const response = await fetch('https://api.unsplash.com/photos/random?client_id=72utkjatCBC-PDcx7-Kcvgod7-QOFAm2fXwEeW8b8cc');
const data = await response.json();
  const url =data.urls.regular
...
adams({ nomCom: "enhance", reaction: "💥", categorie: "User" }, async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      let caption = '*Enhanced by VIRUSI-MBAYA-MDV2*';
